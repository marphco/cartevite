...
export function useEditorInteractions({
  ...
  const handlePointerDown = (e, layer) => {
    ...
    if(canvasRef.current) {
        ...
        newSelected.forEach(id => {
            ...
            if(l) offsets[id] = { startX: l.x, startY: l.y, offsetX: mouseX - l.x, offsetY: mouseY - l.y };
        });
  ...
  const handleResizePointerDown = (e, layer, handlePostion) => {
    ...
    if(canvasRef.current) {
        const canvasRect = canvasRef.current.getBoundingClientRect();
        const scale = canvasRect.width / canvasProps.width;
        let currentX = layer.x, currentY = layer.y;
        ...
        resizeStart.current = {
            ...
            startSize: layer.fontSize, startW: typeof layer.w === 'number' ? layer.w : (document.getElementById(`layer-${layer.id}`)?.offsetWidth || 100),
  ...
  useEffect(() => {
    const handlePointerMove = (e) => {
      ...
      if (pointers.length === 2 && draggingLayerId.current) {
         ...
         if (!pinchStartRef.current) {
            ...
            if (layer) pinchStartRef.current = { initialDist: dist, startSize: layer.type === 'text' || !layer.type ? layer.fontSize : null, startW: layer.type === 'image' ? (layer.w || 100) : null, startH: layer.type === 'image' ? (layer.h || 100) : null };
            return;
         }
         ...
         setLayers(prev => prev.map(l => {
           if (l.id === draggingLayerId.current) {
               if (!l.type || l.type === 'text') {
                 let newSize = Math.max(12, Math.min(pinchStartRef.current.startSize * scaleFactor, 300));
                 return { ...l, fontSize: Math.round(newSize) };
               } else if (l.type === 'image') {
                 let newW = Math.max(20, Math.min(pinchStartRef.current.startW * scaleFactor, canvasProps.width * 2));
                 return { ...l, w: Math.round(newW), h: Math.round(newW * (pinchStartRef.current.startH / pinchStartRef.current.startW)) };
               }
           }
           return l;
         }));
      ...
      if (draggingLayerId.current && dragGeometries.current) {
         setLayers(prev => {
            ...
            const SNAP_THRESHOLD = 5; let newGuides = [], snappedX = false, snappedY = false;
    ...
    const handlePointerUp = (e) => {
      ...
      try { if (e.pointerId && e.target && e.target.hasPointerCapture && e.target.hasPointerCapture(e.pointerId)) e.target.releasePointerCapture(e.pointerId); } catch (e) { console.error("Pointer release error:", e); }
          ...
          if (mxX - mnX > 5 || mxY - mnY > 5) {
             setLayers(prev => {
                const selectedIds = prev.filter(l => {
                    ...
                    const w = node.offsetWidth, h = node.offsetHeight, lx = typeof l.x === 'number' && !isNaN(l.x) ? l.x : canvasProps.width/2, ly = typeof l.y === 'number' && !isNaN(l.y) ? l.y : canvasProps.height/2;
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (editingLayerId || e.target.tagName.toLowerCase() === 'input' || e.target.tagName.toLowerCase() === 'textarea' || e.target.isContentEditable) return;
