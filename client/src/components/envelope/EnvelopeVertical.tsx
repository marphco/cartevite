...
interface EnvelopeVerticalProps {
...
export default function EnvelopeVertical({ 
  ...
  const pH = canvasProps?.height || 500;
  const nativeRatio = pW / pH;
  const dynamicEnvRatio = nativeRatio + 0.1;
  ...
  const getSceneScale = () => {
    ...
    if (windowDims.w <= 768) {
       const topBoundary = 80; // Topbar
       const bottomBoundary = 280; // Mobile toolbar
       ...
       const targetH = (windowDims.h - 150) * 0.85;
       const fitS = targetH / targetHeight;
       return Math.min(fitS, 0.9);
    }
  };
  ...
  const getSceneY = (currentScale: number) => {
    ...
    // Centratura nell'area visibile (sottraendo topbar e toolbar/sidebar)
    const topLimit = 80;
    ...
    // Se la busta è aperta, spostiamo leggermente giù il baricentro per centrare l'intero blocco (tasca + aletta + card)
    const isFullShow = phase !== "closed";
    if (isFullShow) {
  ...
  return (
    <div className={`paperless-wrapper ${phase}`}>
       <div className="envelope-hero" onClick={handleClick}>
         <motion.div 
            ref={sceneRef}
            className={`envelope-3d-scene env-vertical ${isEditingLiner ? 'is-editing-liner' : ''}`}
            initial={{ y: 0, scale: 1 }}
            animate={{ 
